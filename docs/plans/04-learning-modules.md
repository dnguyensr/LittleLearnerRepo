...
- [x] Difficulty tiers: picker in parent settings _and_ auto-progression (advances a tier every 5 correct in a session):
...
- [x] Phonics option: letter _sounds_ instead of letter names (toggle in the parent settings panel).
...
- [x] Grown-up settings panel (`⚙️ Grown-ups`, one tap to open): speech on/off, phonics emphasis, and Math/Words presentation and stage controls. Destructive progress resets retain a separate two-tap confirmation. _(volume control and per-module visibility still open)_
